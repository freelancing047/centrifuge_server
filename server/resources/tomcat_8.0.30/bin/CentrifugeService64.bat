...
set EXECUTABLE=%CATALINA_HOME%\bin\procrun64.exe

if exist "%EXECUTABLE%" goto selectService

echo The tomcat.exe was not found in directory %CATALINA_HOME%\bin...
...
set PR_JVM=
if exist "%CATALINA_HOME%\jre\bin\client\jvm.dll" set PR_JVM=%CATALINA_HOME%\jre\bin\client\jvm.dll 
if exist "%CATALINA_HOME%\jre\bin\server\jvm.dll" set PR_JVM=%CATALINA_HOME%\jre\bin\server\jvm.dll 

if NOT "%PR_JVM%foo"=="foo" goto foundJvm
