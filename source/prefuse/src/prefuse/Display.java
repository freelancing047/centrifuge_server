...
public class Display extends JComponent {
   private static final Logger LOG = LogManager.getLogger(Display.class);
    ...
    public synchronized void setPredicate(Predicate p) {
        if ( p == null ) {
            m_predicate.set(new VisiblePredicate());
        } else {
            m_predicate.set(new Predicate[] {p, new VisiblePredicate()});
        }
    }
    ...
    public RenderingQueue getRenderingQueue()
    {
        return m_queue;
    }
    ...
    public synchronized Point2D transformScreenPointToItem(Point p){
        return (m_itransform==null ? p : 
            m_itransform.transform(p, m_tmpPoint));
    }

    //Should be Point2d for input
    public synchronized Point2D transformItemPointToScreen(Point p) throws NoninvertibleTransformException{
        return (m_itransform==null ? p : 
            m_itransform.inverseTransform(p, m_tmpPoint));
    }
