 ...
 * Barnes-Hut algorithm for efficient n-body force simulations, using a
 ...
 * <ul>
 * <li><a
 * href="http://www.cs.berkeley.edu/~demmel/cs267/lecture26/lecture26.html">James
 * Demmel's UC Berkeley lecture notes</a>
 * <li><a
 * href="http://www.physics.gmu.edu/~large/lr_forces/desc/bh/bhdesc.html">Description
 * of the Barnes-Hut algorithm</a>
 * <li><a
        ...
        extends AbstractForce
{
    ...
    /*
     * The indexing scheme for quadtree child nodes goes row by row. 0 | 1 0 ->
     * top left, 1 -> top right ------- 2 | 3 2 -> bottom left, 3 -> bottom
     * right
     */

    private static String[] pnames = new String[] {
            "GravitationalConstant", "Distance", "BarnesHutTheta"
    };
    ...
    public static final int MAX_QUAD_LEVEL = 20;
     ...
     * 
     * @param gravConstant
     *            the gravitational constant to use. Nodes will attract each
     *            other if this value is positive, and will repel each other if
     *            it is negative.
     * @param minDistance
     *            the distance within which two particles will interact. If -1,
     *            the value is treated as infinite.
     * @param theta
     *            the Barnes-Hut parameter theta, which controls when an
     *            aggregated mass is used rather than drilling down to
     *            individual item mass values.
     */
    public NBodyForce( float gravConstant, float minDistance, float theta )
    {
        ...
        minValues = new float[] {
                DEFAULT_MIN_GRAV_CONSTANT, DEFAULT_MIN_DISTANCE, DEFAULT_MIN_THETA
        };
        maxValues = new float[] {
                DEFAULT_MAX_GRAV_CONSTANT, DEFAULT_MAX_DISTANCE, DEFAULT_MAX_THETA
        };
...
//        root = new QuadTreeNode();
     ...
     * this call has been made, the simulation can be queried for the n-body
     * force acting on a given item.
     * 
     * @param fsim
...
//        root = new QuadTreeNode();
        ...
        long icount = fsim.getItemCount();

        float radius = icount / 4; 
        if (radius < 100) {
            radius = 100;
        }
        while( itemIter.hasNext() ) {
            ForceItem item = (ForceItem)itemIter.next();
            if (Double.isNaN( item.location[0]) ) {
                // randomly place in a circle
//                double r1 = 0f, r2 = 0f, a = 0f; 
//                r1 = rand.nextFloat();
//                a = rand.nextFloat() * (2 * Math.PI);
//                
//                r2 = (radius * Math.sqrt( r1 ));
//                                
//                item.location[0] = (float) (Math.cos( a ) * r2);
//                item.location[1] = (float) (Math.sin( a ) * r2); 
                
                // randomly place nodes in a square
//                item.location[0] = (rand.nextFloat() * 1000); 
//                item.location[1] = (rand.nextFloat() * 1000);   
     ...
     *             if the current location of the item is outside the bounds of
     *             the quadtree
     */
    ...
    private void insert( ForceItem p, QuadTreeNode n, float x1, float y1, float x2, float y2 )
    {
        ...
        if( n.depth > MAX_QUAD_LEVEL ) {
            // don't keep doing recursion, stop here and add up all the force
            // items....
            n.forceValues.add( p );
        } else {
            if( n.hasChildren ) {
    ...
    private void calcMass( QuadTreeNode n )
    {
        ...
        if( n.depth > MAX_QUAD_LEVEL ) {
            Iterator iterator = n.forceValues.iterator();
            while( iterator.hasNext() ) {
                ForceItem fi = (ForceItem)iterator.next();
                n.mass += fi.mass;
                xcom += fi.mass * fi.location[ 0 ];
                ycom += fi.mass * fi.location[ 1 ];
            }
        }
        if( n.value != null ) { 
            n.mass += n.value.mass;
...
//        if( ( !n.hasChildren && (n.value != item || !n.forceValues.contains( item )) ) || ( !same && ( x2 - x1 ) / r < params[ BARNES_HUT_THETA ] ) ) {
        if( ( !n.hasChildren && (n.value != item) ) || ( !same && ( x2 - x1 ) / r < params[ BARNES_HUT_THETA ] ) ) {
            ...
            for( int i = 0; i < n.children.length; i++ ) {
                if( n.children[ i ] != null ) {
                    forceHelper( item, n.children[ i ], ( i == 1 || i == 3 ? splitx : x1 ), ( i > 1 ? splity : y1 ),
    ...
    public static final class QuadTreeNode
    {
        ...
        List forceValues = new ArrayList();
    ...
    /**
     * Helper class to minimize number of object creations across multiple uses
     * of the quadtree.
     */
