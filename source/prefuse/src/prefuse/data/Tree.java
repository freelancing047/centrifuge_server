...
public class Tree extends Graph {
   private static final Logger LOG = LogManager.getLogger(Tree.class);
    ...
    public boolean isValidTree() {
        ...
        if ( nnodes != nedges+1 ) {
           LOG.warn("Node/edge counts incorrect.");
            return false;
        }
        ...
        while ( nodes.hasNext() ) {
            ...
            if ( n==root && id > 0 ) {
               LOG.warn("Root node has a parent.");
                return false;
            } else if ( id > 1 ) {
               LOG.warn("Node "+n+" has multiple parents.");
                return false;
            }
        }
        ...
        if ( counts[0] > nedges ) {
           LOG.warn("The tree has non-tree edges in it.");
            return false;
        }
        if ( counts[0] < nedges ) {
           LOG.warn("Not all of the tree was visited. " +
