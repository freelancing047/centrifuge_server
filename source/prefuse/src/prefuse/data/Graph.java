...
public class Graph extends CompositeTupleSet {
    ...
    public void removeAllListeners() {
    	removeListeners((Table) getSet(NODES));
    	removeListeners((Table) getSet(EDGES));
    	
    	m_listener = null;
    }
    
    private void removeListeners(Table table) {
        for (int i = 0; i < table.getColumnCount(); i++) {
        	Column col = table.getColumn(i);
//        	col.removeColumnListener(m_listener);
        	col.removeAllColumnListener();
        } 
        
//        table.removeTableListener(m_listener);
        table.removeAllTableListener();
    }    
