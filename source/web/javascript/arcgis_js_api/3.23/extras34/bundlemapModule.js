define([
    ...
    function initMap() {
        ...
        map.on("click", function(evt) {
            ...
            if (evt.ctrlKey === true || evt.metaKey === true || selectPoint || (!drilledToBottom && !showLeaves)) {
                ...
                if (evt.ctrlKey === true || evt.metaKey === true) {
                    ...
                    !map.isPan && map.enablePan();
                    !map.isRubberBandZoom && map.enableRubberBandZoom();
                } else if (!drilledToBottom && !showLeaves) {
                    drillOnBundle(response);
                }
            }
        });
    ...
    var oldFeatureLayer = null;
    var oldPlacesLayer = null;
    var oldLabelLayer = null;
    ...
    function useData(jsonFS) {
        if (jsonFS == null || jsonFS.deferToNewCache) {
            ...
            window.parent.setUseSummary(vizid, false);
    ...
    function initializeValues(jsonFS) {
        ...
        currentSize += increment;
        currentSizes.push(currentSize);
        drilledToBottom = jsonFS.drilledToBottom;
        showLeaves = jsonFS.showLeaves;
        if (showLeaves)
            window.parent.showBackButton(vizid);
        else
            window.parent.hideBackButton(vizid);
        if (drilledToBottom || showLeaves) {
            if (updateLegend) {
    ...
    function itemsExtentsReload() {
        if (window.parent.isMapPinned(vizid)) {
            refreshAndUpdateMap(map.extent, map.getZoom());
        } else {
            xhr.get({
                url: "rest/map/getExtentInfo/" + qs("dv_uuid") + "/" + qs("viz_uuid") + "/" + getNextSequenceNumber(),
                handleAs: "json",
                load: function(newExtentInfo) {
                    var zoomAndExtent = extentFunction.calculateZoomAndExtent(newExtentInfo.initialExtent, map.extent.spatialReference.wkid);
    ...
    function initDialog() {
        ...
        on(dialog, "click", function(evt) {
            if (evt.ctrlKey === true || evt.metaKey === true || (!drilledToBottom && !showLeaves)) {
                var screenPoint = new ScreenPoint(evt.clientX - extentFunction.getNewPaddingLeft(), evt.clientY);
                var mapPoint = map.toMap(screenPoint);
                currentDisplayPoint = mapPoint;
                currentQueryPoint1 = mapPoint;
    ...
    function featureLayerContextMenu(response) {
        ...
        if (topmostFeature != null) {
            ...
            if (!drilledToBottom && !showLeaves) {
                showPointsMenuItem.parentElement.style.display = "inline";
            } else {
                showPointsMenuItem.parentElement.style.display = "none";
            }
    ...
    function highlightDialogOn() {
        ...
        var highlightSymbol = symbolFunction.createPlaceSymbol(dialogOnFeature.attributes.typeId, useIcon, iconUri, iconSize, isSelected, ishighlighted, false, isCombined, useMultitypeDecorator, isNew, isUpdated, useLinkupDecorator, 1);
            ...
            while (i < jsonFS.layer.associations.length) {
                ...
                if (symbolFunction.foundLineSymbol(a)) {
                    ...
                    if (prepareLineDefinition(featureHash, linkHash, sourceGeometries, destinationGeometries, arr, a)) {
                        var lineDefinition = symbolFunction.createLineDefinition(map, a, sourceGeometries, destinationGeometries, symbolFunction.isLineShowDirection(a.typeId), iXMin, iXMax);
    ...
    function setFeatureLayer(jsonFS) {
        if (!jsonFS.showLabel) {
            return;
        }
        var layerDefinition = {
        ...
        var labelColor = new Color("#000");
        // create a text symbol to define the style of labels
        var labelSymbol = new TextSymbol().setColor(labelColor);
        labelSymbol.font.setFamily("arial");
        labelSymbol.font.setWeight(Font.WEIGHT_BOLDER);
        labelSymbol.setAlign(TextSymbol.ALIGN_START);
        labelSymbol.setHaloColor(new Color([255, 255, 255]));
        labelSymbol.setHaloSize(1);
        var labelRenderer = new SimpleRenderer(labelSymbol);
        var labels = new LabelLayer();
        labels.className = "unselectable";
        var labelOptions = {
            pointPriorities: "AboveCenter"
        };
        labels.addFeatureLayer(featureLayer, labelRenderer, "{label}", labelOptions);
        labelLayer = labels;
    }

    function deselectFunction() {
        dataFunction.deselectInBuffer2(rightMenuOnKey);
        rightMenuOnKey = null;
        cmenu.style.display = "none";
    }

    function selectFunction() {
        dataFunction.selectInBuffer2(rightMenuOnKey);
        rightMenuOnKey = null;
        cmenu.style.display = "none";
    }
    ...
    function prepareDownloadPlaceSymbols(jsonFS, alpha) {
        ...
        while (i < jsonFS.layer.features.length) {
            ...
            symbolFunction.prepareDownloadPlaceSymbol(f.attributes.typeId, useIcon, iconUri, iconSize, isSelected, ishighlighted, false, isCombined, useMultitypeDecorator, isNew, isUpdated, useLinkupDecorator, alpha);
            i++;
        }
    }
    ...
    function createPlacesLayer(jsonFS, alpha) {
        ...
        while (i < jsonFS.layer.features.length) {
            ...
            var isNew = getIsSet(f, "new");
            var isUpdated = getIsSet(f, "updated");
            var placeSymbol = symbolFunction.createPlaceSymbol(f.attributes.typeId, useIcon, iconUri, iconSize, isSelected, ishighlighted, false, isCombined, useMultitypeDecorator, isNew, isUpdated, useLinkupDecorator, alpha);
    ...
    function updatePlaceGraphics(f) {
        ...
        var isNew = getIsSet(f, "new");
        var isUpdated = getIsSet(f, "updated");
        var placeSymbol = symbolFunction.createPlaceSymbol(f.attributes.typeId, useIcon, iconUri, iconSize, isSelected, ishighlighted, false, isCombined, useMultitypeDecorator, isNew, isUpdated, useLinkupDecorator, 1);
        f.setSymbol(placeSymbol);
    }
    var showPointsMenuItem;
    var drilledToBottom;
    var showLeaves;

    function drillOnBundle(response) {
        var features = response.features;
        var filteredFeatures = [];
        for (var i = 0; i < features.length; i++) {
            feature = features[i];
        ...
        if (filteredFeatures.length > 0) {
            drill(filteredFeatures);
        }
    }

    function drill(features) {
        closeDialog();
        window.parent.mapViewShowLoading(vizid);
        map.disableMapNavigation();
        ...
        xhr.get({
            url: "rest/map/isDrillable/" + dvid + "/" + vizid + "/" + getNextSequenceNumber() + "/" + objectID,
            handleAs: "json",
            load: function(result) {
                ...
                if (result == "true" || result == true) {
                    ...
                    dojo.xhrPost({
                        url: "rest/map/drillOnBundle/" + dvid + "/" + vizid + "/" + getNextSequenceNumber(),
                        postData: data,
                        handleAs: "text",
                        load: function(data) {
                            if (data == 'success') {
                    ...
                    window.parent.tooManyPoints(vizid, result);
                }
            }
        });
    }

    function drillOnBundleOnDialog(response) {
        var features = response.features;
        var filteredFeatures = [];
        for (var i = 0; i < features.length; i++) {
            feature = features[i];
    ...
    function handleBundling(jsonFS) {
        var breadcrumbAggregate = jsonFS.breadcrumb.join(";");
        var showLeave = jsonFS.showLeaves ? "true" : "false";
        var drilledToBottom = jsonFS.drilledToBottom ? "true" : "false";
        window.parent.handleBreadcrumb(vizid, breadcrumbAggregate, showLeave, drilledToBottom);
    }
    ...
    function showPointsFunction() {
        ...
        window.parent.mapViewShowLoading(vizid);
        map.disableMapNavigation();
        var objectID = rightMenuOnKey[0];
        xhr.get({
            url: "rest/map/isUnderlyingPointsWithinLimits/" + dvid + "/" + vizid + "/" + getNextSequenceNumber() + "/" + objectID,
            handleAs: "json",
            load: function(result) {
                window.parent.mapViewHideLoading(vizid);
                map.enableMapNavigation();
                ...
                if (result == "true" || result == true) {
                    ...
                    dojo.xhrPost({
                        url: "rest/map/showLeavesOnBundle/" + dvid + "/" + vizid + "/" + getNextSequenceNumber(),
                        postData: data,
                        handleAs: "text",
                        load: function(data) {
                            if (data == 'success') {
    ...
    return declare(commonModule, {
        ...
        constructor: function(params) {
            ...
            showPointsMenuItem = dom.byId(params.showPointsId);
            on(showPointsMenuItem, "click", showPointsFunction);
        ...
        backToBundle: function() {
            window.parent.mapViewShowLoading(vizid);
            map.disableMapNavigation();
            dojo.xhrPost({
                url: "rest/map/dontShowLeavesOnBundle/" + dvid + "/" + vizid + "/" + getNextSequenceNumber(),
                handleAs: "text",
                load: function(data) {
                    window.parent.mapViewHideLoading(vizid);
                    map.enableMapNavigation();
        ...
        trimBreadcrumb: function(columnName) {
            window.parent.mapViewShowLoading(vizid);
            map.disableMapNavigation();
            dojo.xhrPost({
                url: "rest/map/trimBreadcrumb/" + dvid + "/" + vizid + "/" + getNextSequenceNumber(),
                postData: columnName,
                handleAs: "text",
                load: function(data) {
                    window.parent.mapViewHideLoading(vizid);
