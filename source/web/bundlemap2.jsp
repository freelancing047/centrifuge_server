...
<%
    ...
    String showPointsText = HtmlUtils.htmlEscape(properties.get("mapViewShowpoints"));
...
%>
		...
		var locatorUrl = "<%= mapConfig.getLocatorUrl() %>";
                ...
                window.addEventListener("message", function(event) {
                    if (mapModuleLoaded) {
                        if (event.data == "reload") {
                            ...
                            toggleSearchHandler();
                        } else if (event.data == "backToBundle") {
                            backToBundleHandler();
                        } else if (event.data.startsWith("trimBreadcrumb")) {
                            var arr = event.data.split(";");
                            var columnName = arr[1];
                            trimBreadcrumbHandler(columnName);
                ...
                require(['extras34/bundlemapModule', 'extras34/Exporter', 'extras34/geometryService', 'dojo/_base/xhr', "esri/SpatialReference", "esri/geometry/Point", "esri/geometry/Extent", "dojo/domReady!"], function(MapModule, Exporter, GeometryService, xhr, SpatialReference, Point, Extent) {
			    ...
			    isNewExtent: isNewExtent
                        ...
                        backToBundleHandler = function() {
                            mapModule.backToBundle();
                        }
                        trimBreadcrumbHandler = function(columnName) {
                            mapModule.trimBreadcrumb(columnName);
                        }
            ...
            <div id="cmenu" class='popupContent' style="display:none;">
                <div>
                    <ul class='dropdown-menu' style='display: block;'>
                        ...
                        <li style="display:none;"><a id="showpoints"><%= showPointsText %></a></li>
                    </ul>
                </div>
            </div>
        </div>
