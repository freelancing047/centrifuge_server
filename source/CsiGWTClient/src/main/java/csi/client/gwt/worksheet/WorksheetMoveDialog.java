...
public class WorksheetMoveDialog {
	
    public static CentrifugeConstants i18n = CentrifugeConstantsLocator.get();

    private final String ACTION_LABEL = i18n.worksheetMoveDialogActionButton(); //$NON-NLS-1$

	private Dialog dialog;

    @UiField(provided = true)
    String title = i18n.worksheetMoveDialogTitle(); //$NON-NLS-1$
    ...
    interface SpecificUiBinder extends UiBinder<Dialog, WorksheetMoveDialog> {
    }

    private static SpecificUiBinder uiBinder = GWT.create(SpecificUiBinder.class);

    public WorksheetMoveDialog(WorksheetPresenter worksheetPresenter, String uuid) {
        super();
        presenter = worksheetPresenter;
        vizUuid = uuid;
        dialog = uiBinder.createAndBindUi(this);
        ListStore<String> listStore = new ListStore<>(String::toString);

        List<String> names = worksheetPresenter.getWorksheetNames();
        names.remove(worksheetPresenter.getName());

        listStore.addAll(names);

        //worksheetList = new ComboBoxCell<String>(listStore, new StringLabelProvider());
        ...
        addClickHandler(event -> {
            if (isValid()) {
                presenter.move(vizUuid, getName());
                hide();
            }
        });
    }
