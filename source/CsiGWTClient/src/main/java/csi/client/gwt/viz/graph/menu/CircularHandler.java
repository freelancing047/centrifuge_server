...
public class CircularHandler extends AbstractMenuEventHandler<Graph, GraphMenuManager> {
    public CircularHandler(Graph graph, GraphMenuManager mgr) {
        super(graph, mgr);
    }
    ...
    public void onMenuEvent(CsiMenuEvent event) {
        if (getPresenter().isViewLoaded()) {
            ...
            VortexFuture<List<CsiMap<String, String>>> future = getPresenter().getModel().applyLayout(GraphLayout.circular);
        	...
        	getPresenter().getModel().applyLayoutBeforeLoad(GraphLayout.circular);
        	getPresenter().getModel().checkLayout(GraphLayout.circular);
        }
    }
}
