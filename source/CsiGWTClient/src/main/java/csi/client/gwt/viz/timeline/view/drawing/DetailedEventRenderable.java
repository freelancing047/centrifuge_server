...
public class DetailedEventRenderable extends EventRenderable{
    ...
    private DetailedEventProxy event;
    ...
    public DetailedEventRenderable(DetailedEventProxy event, TimelineTrackModel trackModel){
        this.event = event;
        event.setTrack(trackModel);
    }
...
//        }
        ...
        context2d.beginPath();
        double r = getDotR();
        if(getTrack().hasSummary() && getTrack().isVisible()){
        ...
        if(!getTrack().isCollapsed() && getTrack().isVisible()){
            if(event.isSelected()){
...
//            if(event.getX() )
            if(showLabel){
                ...
                if (labelSpace>20)
                {
                    ...
                    String s= format(label, labelSpace/7, false);
    ...
    private void drawSummaryIfPossible(Context2d context2d) {
        ...
        int y = getTrack().getEndY() - TrackRenderable.FULL_SUMMARY_HEIGHT;
        if(y > viewPort.getCurrentHeight() - viewPort.getStart() || y < 0){
            return;
        }
    ...
    public void drawEvent(Context2d context2d, int y, int ox, double r) {
        ...
        if(getEvent().getEvent().getColorValue() == null || getEvent().getEvent().getColorValue() == TimelineTrack.NULL_TRACK){
            ...
            Color color = ClientColorHelper.get().make(getEvent().getColor());
            
            CssColor cssColor = CssColor.make(color.getRed(), color.getGreen(), color.getBlue());
            context2d.setFillStyle(cssColor);
            context2d.setStrokeStyle(cssColor); 
        }
        ...
        if (hasEnd() && isShowDuration() && validateEnd())
        {
            ...
            context2d.fillRect(getX() + 2*r, lineY, getEndX()-getX(), 2);
...
//            context2d.setLineWidth(1);
    ...
    public int compareEnd(DetailedEventRenderable eventRenderable){
    ...
    public DetailedEventProxy getEvent() {
        return event;
    }

    public void setEvent(DetailedEventProxy event) {
        this.event = event;
    }
