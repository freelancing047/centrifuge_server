...
package com.yahoo.astra.mx.controls
{
	 ...
	 * @default true
	 */
	[Style(name="showMilliseconds", type="Boolean", inherit="no")]
	...
	//--------------------------------------
		...
		{
			return textFields[0].text + ":" + textFields[1].text + ":" + textFields[2].text + "." + textFields[3].text + " " + textFields[4].text;
		}
	...
	//--------------------------------------
		...
		override public function styleChanged(styleProp:String):void
		{
			...
			if(!styleProp || styleProp == "showSeconds" || styleProp == "showMilliseconds" || styleProp == "showAMPM" 
			    || styleProp == "displayTwoDigitHoursValue" || styleProp == "useTwelveHourFormat" 
			    || styleProp == "AMText" || styleProp == "PMText")
			{
				this.invalidateProperties();
				this.invalidateSize();
				this.invalidateDisplayList();
	...
	//--------------------------------------
		...
		override protected function createChildren():void
		{
			...
			var milliSeparatorField:UITextField = new UITextField();
			milliSeparatorField.styleName = this;
			milliSeparatorField.ignorePadding = true;
			milliSeparatorField.selectable = false;
			milliSeparatorField.enabled = this.enabled;
			milliSeparatorField.text = ".";
			this.addChild(milliSeparatorField);
			this.separators.push(milliSeparatorField);
			...
			var millisecondsField:UITextField = UITextField(this.textFields[3]);
			millisecondsField.styleName=this;
			millisecondsField.ignorePadding = true;
			millisecondsField.maxChars = 3;
			millisecondsField.restrict = "0-9";
			millisecondsField.addEventListener(FocusEvent.FOCUS_IN, millisecondsFocusInHandler);
			millisecondsField.addEventListener(FocusEvent.FOCUS_OUT, millisecondsFocusOutHandler);
			millisecondsField.addEventListener(TextEvent.TEXT_INPUT, millisecondsTextInputHandler);

			var AMPMField:UITextField = UITextField(this.textFields[4]);
		...
		override protected function commitProperties():void
		{
			...
			var showMilliseconds:Boolean = this.getStyle("showMilliseconds");
			...
			for(var i:int = 0; i < separatorCount; i++)
			{
				...
				if(i == 2){
					separator.visible = showMilliseconds;
				}
			}
			...
			var millisecondsField:UITextField = UITextField(this.textFields[3]);
			millisecondsField.text = StringUtil.padFront(this.value.getMilliseconds().toString(), 3, "0");
			millisecondsField.visible = showMilliseconds;
		...
		override protected function measure():void
		{
			...
			{
				var separator:UITextField = UITextField(this.separators[i]);
				if(i == 0 || (i == 1 && showSeconds) || (i == 2 && showMilliseconds))
				{
			...
			{
				var millisecondsField:UITextField = UITextField(this.textFields[3]);
				this.measuredWidth += this._numberWidth * 1.5;
			}
		...
		override protected function layoutTextFields():void
		{
			...
			if (showMilliseconds)
			{
				separatorField = UITextField(this.separators[2]);
				separatorField.x = xPosition;
				separatorField.y = yPosition;
				separatorField.setActualSize(Math.min(Math.max(0, unscaledWidth - metrics.right - separatorField.x), separatorField.measuredWidth), tfHeight);
				xPosition += separatorField.measuredWidth;
				...
				var millisecondsField:UITextField = UITextField(this.textFields[3]);
				millisecondsField.x = xPosition;
				millisecondsField.y = yPosition;
				millisecondsField.setActualSize(Math.min(Math.max(0, unscaledWidth - metrics.right - xPosition), this._numberWidth*1.5), tfHeight);
				xPosition += millisecondsField.width;
			}
		...
		override protected function focusNextTextField(textField:UITextField):void
		{
			...
			{
				//if we aren't showing seconds, but we're showing milliseconds, switch to the milliseconds 
				//field.  If we're not showing seconds or milliseconds, but we are showing AM/PM, switch
				...
				if(showMilliseconds)
				{
					...
					this.callLater(UITextField(this.textFields[4]).setFocus);
				}
				return;
			}
			else if (index == 2 && !showMilliseconds){
				//If we're not showing milliseconds, but we are showing AM/PM, switch
				//to the AMPMField. otherwise, ignore the request.
				if (showAMPM){
					this.callLater(UITextField(this.textFields[4]).setFocus);
				}
			...
			else if(index == 3 && (!useTwelveHourFormat || !showAMPM))
			{
		...
		override protected function focusPreviousTextField(textField:UITextField):void
		{
			...
			else if (index == 4 && !showMilliseconds)
			{
				if (showSeconds){
					this.callLater(UITextField(this.textFields[2]).setFocus);
				}
				else{
					this.callLater(UITextField(this.textFields[1]).setFocus);
				}
	...
	//--------------------------------------
		...
		/**
		 * @private
		 * Validates milliseconds input.
		 */
		protected function millisecondsTextInputHandler(event:TextEvent):void
		{
			var textField:UITextField = UITextField(event.target);
			...
			if(value > 999)
			{
				//if a value is greater than 999, we don't want it!
				event.preventDefault();
			}
		}
		...
		/**
		 * @private
		 * Informs listeners when the milliseconds field receives focus.
		 */
		protected function millisecondsFocusInHandler(event:FocusEvent):void
		{
			this.dispatchEvent(new TimeInputEvent(TimeInputEvent.MILLISECONDS_FOCUS_IN));
		}
		
		/**
		 * @private
		 * Handles changes to the milliseconds UITextField. Updates the dataProvider.
		 */
		protected function millisecondsFocusOutHandler(event:FocusEvent):void
		{
			var millisecondsField:UITextField = UITextField(event.target);
			
			var milliseconds:Number = 0;
			if(millisecondsField.length > 0)
			{
				milliseconds = int(millisecondsField.text);
			}
			//validate
			milliseconds = Math.min(milliseconds,999);
