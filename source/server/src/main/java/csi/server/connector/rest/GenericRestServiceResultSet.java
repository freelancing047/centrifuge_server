...
/*
 *  IMPL notes:
 *  Make sure that data access is always done with names!  Any request that comes in for
 *  ordinal position requires a look up for the name, then a
 */
public class GenericRestServiceResultSet implements ResultSet {
   ...
   class ElementRetrieval implements IValueRetrieval {
    ...
    private List<IValueRetrieval> internalSchema;
    ...
    GenericRestServiceResultSet(NodeList records) throws SQLException {
        this.records = records;
        discoverMetaData();

    }

    protected void discoverMetaData() throws SQLException {
        internalSchema = new ArrayList<IValueRetrieval>();
        rsmd = new RowSetMetaDataImpl();
        rsmd.setColumnCount(1);
        rsmd.setColumnName(1, "Dummy");
        rsmd.setColumnType(1, Types.VARCHAR);
        names = new ArrayList<String>();

        if (records.getLength() > 0) {
            Node node = records.item(0);
            NamedNodeMap attributes = node.getAttributes();
            if (attributes != null) {
               int howMany = attributes.getLength();

               for (int i = 0; i < howMany; i++) {
                    Node item = attributes.item(i);
                    String name = item.getNodeName();
                    AttributeRetrieval retrieval = new AttributeRetrieval(name);
                    internalSchema.add(retrieval);
                    names.add(name);
                }
            }

            NodeList childNodes = node.getChildNodes();
            int howMany = childNodes.getLength();
            for (int i = 0; i < howMany; i++) {
                Node item = childNodes.item(i);
                if (item.getNodeType() == Node.ELEMENT_NODE) {
                    // make sure we don't have multiples!
                    String tag = item.getNodeName();
                    if (!names.contains(tag)) {
                        names.add(tag);
                        ElementRetrieval elementRetrieval = new ElementRetrieval(tag);
                        internalSchema.add(elementRetrieval);
                    }
                }
            }
            ...
            try {
               howMany = names.size();
                rsmd.setColumnCount(howMany);

                for (int i = 0; i < howMany; i++) {
                    rsmd.setColumnLabel(i + 1, names.get(i));
                    // rsmd.setColumnName( i + 1, names.get( i ) );
                    // TODO ADD HERE
                    rsmd.setColumnName(i + 1, names.get(i));
    ...
    public Object getObject(int columnIndex) throws SQLException {
        IValueRetrieval retrieval = internalSchema.get(columnIndex - 1);
        Node item = records.item(currentPosition);
        Object value = retrieval.getValue(item);
        return value;
    }
    ...
    public String getString(String columnLabel) throws SQLException {
        int indexOf = names.indexOf(columnLabel);
        IValueRetrieval retrieval = internalSchema.get(indexOf);
        Node record = records.item(currentPosition);
        Object data = ((retrieval != null) ? retrieval.getValue(record) : null);
        return (String) data;
    }
