 ...
 */
public class CsiForceDirectedLayout extends Layout {
     ...
     */
    public CsiForceDirectedLayout(String graph) {
     ...
     */
    public CsiForceDirectedLayout(String group, boolean enforceBounds) {
     ...
     * layout is computed.
     */
    public CsiForceDirectedLayout(String group, boolean enforceBounds, boolean runonce) {
        super(group);
     ...
     * that all node placements stay within the layout bounds.
     */
    public CsiForceDirectedLayout(String group, ForceSimulator fsim, boolean enforceBounds) {
     ...
     * layout is computed.
     */
    public CsiForceDirectedLayout(String group, ForceSimulator fsim, boolean enforceBounds, boolean runonce) {
        super(group);
        m_nodeGroup = PrefuseLib.getGroupName(group, Graph.NODES);
        m_edgeGroup = PrefuseLib.getGroupName(group, Graph.EDGES);
    ...
    public void run(double frac) {
        ...
        if (m_runonce) {
            ...
            for (int i = 0; i < m_iterations; i++) {
                ...
                if ((i % 10) == 0) {
                    int percentage = (int) (((float) i / (float) m_iterations) * 100);
                    TaskHelper.reportProgress(percentage);
                }
            ...
            if (BigDecimal.valueOf(frac).compareTo(BigDecimal.ONE) == 0) {
                reset();
            }
